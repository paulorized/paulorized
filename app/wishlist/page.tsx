...
function StrainCard({ item, onRemove }: { item: WishlistItem; onRemove: (id: string, name: string) => void }) {
  ...
  return (
    <div className={`relative rounded-2xl border ${colors.border} ${colors.bg} shadow-lg ${colors.glow} p-4 flex flex-col gap-3 transition hover:scale-[1.02] group`}>
      {/* Dex number */}
      ...
      {items.length > 0 && (
        <div className="mb-6 rounded-2xl border border-zinc-800 bg-zinc-900/60 px-5 py-4">
          ...
          <div className="flex gap-1 h-2 rounded-full overflow-hidden bg-zinc-800">
            {counts.indica > 0 && (
              <div className="bg-purple-500 transition-all" style={{ width: (counts.indica / items.length * 100) + '%' }} />
            )}
            {counts.sativa > 0 && (
              <div className="bg-yellow-400 transition-all" style={{ width: (counts.sativa / items.length * 100) + '%' }} />
            )}
            {counts.hybrid > 0 && (
              <div className="bg-emerald-400 transition-all" style={{ width: (counts.hybrid / items.length * 100) + '%' }} />
            )}
            {counts.unknown > 0 && (
              <div className="bg-zinc-500 transition-all" style={{ width: (counts.unknown / items.length * 100) + '%' }} />
            )}
          </div>
