...
function StrainDetailInner() {
  ...
  useEffect(() => {
    if (!slug) return;
    // First check our own strains index, then fall back to strain-search
    async function load() {
      setLoading(true);
      try {
        // Try index first
        ...
        if (idxRes.ok) {
          ...
          if (idxData.result) { setResult(idxData.result); setLoading(false); return; }
        }
        // Fallback: strain-search by name derived from slug
        ...
        if (searchData.result && !searchData.result.not_found) {
  ...
  if (loading) {
    return (
      <div className="mx-auto w-full max-w-2xl px-4 py-8 space-y-4 animate-pulse">
        ...
        <div className="h-4 w-4/5 rounded-lg bg-zinc-800" />
      </div>
    );
  }
  ...
  if (!result) {
    return (
      <div className="mx-auto w-full max-w-2xl px-4 py-8 text-center space-y-3">
        <div className="text-3xl">🔍</div>
        <p className="text-sm font-medium text-zinc-300">Strain not found</p>
        <button onClick={() => router.back()} className="text-xs text-emerald-400 hover:underline">← Go back</button>
      </div>
    );
  }
